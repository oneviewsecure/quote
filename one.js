...
function generatePDFWithJSPDF(data) {
  ...
  // Company Header (One View Secure Technologies)
  ...
  doc.text("One View Secure Technologies", 105, 14, { align: "center" })
  ...
  // Customer Details
      ...
      // Try to load and draw the service-booking-app.webp image to the right of customer details
      const imageSrc = "images/service-booking-app.webp"
      ...
      // We'll attempt to load the image but not block PDF creation if it fails
      let imagePromise = loadImageAsPngDataUrl(imageSrc).catch((err) => {
  ...
  // Table Header
      ...
      // Wait for imagePromise, then add image on the right of customer details if available, then add footer and resolve
      imagePromise.then((imgDataUrl) => {
        if (imgDataUrl) {
          try {
            // Place image to the right side of Customer Details and just below the Quotation Number
            const pageWidth = doc.internal.pageSize.getWidth()

            // Quotation number is drawn around y=70; place image a few mm below it
            const quotationNumberY = 70
            const topPadding = 4
            const y = quotationNumberY + topPadding

            // Max width for image (keeps some right margin)
            ...
            // X position: center image horizontally on the page
            const x = (pageWidth - imgW) / 2

            // Draw the image (will occupy space on the right beside customer details)
